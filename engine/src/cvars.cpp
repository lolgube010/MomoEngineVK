...
namespace momo_cvars
{
    ...
    struct CVarArray
    {
        ...
        CVarArray(const size_t aSize)
        { 
            cvars = new CVarStorage<T>[aSize]();
        }
        ~CVarArray()
        { 
            delete cvars;
        ...
        int Add(const T& aInitialValue, const T& aCurrentValue, CVarParameter* aParam)
        {
            ...
            cvars[index].current = aCurrentValue;
            cvars[index].initial = aInitialValue;
            cvars[index].parameter = aParam;

            aParam->arrayIndex = index;
            lastCVar++;
            return index;
        }
    };
    ...
    public:
        ...
        void SetCVarCurrent(const uint32_t aNameHash, const T& aValue)
        {
            ...
            if (cvar)
            {
                GetCVarArray<T>()->SetCurrent(aValue, cvar->arrayIndex);
            }
        }
...
double* CVarSystemImpl::GetFloatCVar(momo_stringUtils::StringHash aHash) {return GetCVarCurrent<double>(static_cast<uint32_t>(aHash)); }
    ...
    void CVarSystemImpl::DrawImGuiEditor()
    {
        ...
        if (cachedEditParameters.size() > 10)
        {
            ...
            {
                // alphabetical sort
                std::sort(parameters.begin(), parameters.end(), [](CVarParameter* A, CVarParameter* B) { return A->name < B->name; });
        ...
        {
            // alphabetical sort
            std::sort(cachedEditParameters.begin(), cachedEditParameters.end(), [](CVarParameter* A, CVarParameter* B) { return A->name < B->name; });
            float maxTextWidth = 0;
            for (auto p : cachedEditParameters)
            {
                maxTextWidth = std::max(maxTextWidth, ImGui::CalcTextSize(p->name.c_str()).x);
